...
Change in event freq (ACh - Ctrl) - all slices
```{r}
...
Distribution of event freq changes for each network
```{r}
...
plot.cluster.assignments = function(clust.df, clust.distance=50) {
  ...
  clust.df$cell_id = as.integer(clust.df$cell_id)
...
all.data.binned = all.data.binned.tmp %>%
   ddply(.(animal, exp, bin, cell_id), summarize, 
...
animal.data.binned = all.data.binned.tmp %>%
  ddply(.(animal, exp, bin), summarize, 
...
pop.events.signal = left_join(filter(timebin.isevent, exceeds.thresh),
                              all.data.binned, by=c('animal', 'exp', 'bin'))
...
wilcox.test(events.dur ~ exp, subset(pop.active.summary, exp != 'Atr'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$events.dur, 
              subset(pop.active.summary,exp == 'Ctrl')$events.dur)
wilcox.test(events.dur ~ exp, subset(pop.active.summary, exp != 'Ctrl'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$events.dur, 
              subset(pop.active.summary,exp == 'Atr')$events.dur)
...
wilcox.test(events.freq ~ exp, subset(pop.active.summary,exp != 'Atr'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$events.freq, 
              subset(pop.active.summary,exp == 'Ctrl')$events.freq)
wilcox.test(events.freq ~ exp, subset(pop.active.summary,exp != 'Ctrl'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$events.freq, 
              subset(pop.active.summary,exp == 'Atr')$events.freq)
...
wilcox.test(median.strace ~ exp, subset(pop.active.summary, exp != 'Atr'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$median.strace, 
              subset(pop.active.summary, exp == 'Ctrl')$median.strace)
wilcox.test(median.strace ~ exp, subset(pop.active.summary, exp != 'Ctrl'), paired=TRUE)
wilcox.test.w(subset(pop.active.summary, exp == 'ACh')$median.strace, 
              subset(pop.active.summary, exp == 'Atr')$median.strace)
...
## Cell transient events
Stats only on slices with population events
Frequency of events
```{r}
filtered.animal.freqs = filter(animal.freqs, animal %in% has.pop.event)

filtered.animal.freqs %>%
  group_by(exp) %>%
  dplyr::summarise(mean(med.event.freq), sem(med.event.freq), n=n())

wilcox.test(subset(filtered.animal.freqs, exp=='Ctrl')$med.event.freq,
            subset(filtered.animal.freqs, exp=='ACh')$med.event.freq, paired = TRUE)
wilcox.test.w(subset(filtered.animal.freqs, exp=='ACh')$med.event.freq,
              subset(filtered.animal.freqs, exp=='Ctrl')$med.event.freq)
```
Duration of events
```{r}
filtered.animal.durs = filtered.events.data %>% 
  group_by(animal, exp, animal_exp) %>%
  dplyr::summarise(med.event.dur = median(dur), nevents = n())  %>%
  filter(animal %in% has.pop.event)

filtered.animal.durs %>%
  group_by(exp) %>%
  dplyr::summarise(mean(med.event.dur), sem(med.event.dur), n=n())

wilcox.test(subset(filtered.animal.durs, exp=='Ctrl')$med.event.dur,
            subset(filtered.animal.durs, exp=='ACh')$med.event.dur, paired = TRUE)
wilcox.test.w(subset(filtered.animal.durs, exp=='ACh')$med.event.dur,
              subset(filtered.animal.durs, exp=='Ctrl')$med.event.dur)
```
