...
summary.event.freq.df %>% 
  ...
  dplyr::summarise(freq.med=mean(med.freq), freq.sem=sem(med.freq))
```
...
mean(median.fdiff$m.freq.x)
sem(median.fdiff$m.freq.x)
mean(median.fdiff$m.freq.y)
sem(median.fdiff$m.freq.y)
...
event.dur.ratio %>% group_by(animal) %>%
  dplyr::summarise(mdur.mean.ratio = median(dur.mean.ratio),
...
summary.event.dur.line %>% 
  group_by(animal_line) %>% 
  dplyr::summarise(freq.med=mean(med.event.dur), freq.sem=sem(med.event.dur))
```
...
Change in correlation of the cell pairs
```{r}
...
calc.cor.changes.pct = function(data) {
  cor.ach.data = filter(data, exp=='ACh') %>% get.cor
  cor.ctrl.data = filter(data, exp=='Ctrl') %>% get.cor
...
animal.pop.event.thr = list(
  ...
  Yu_Thy1_1 = list(Ctrl=0.7, ACh=0.7, Atr=0.7),
  Yu_Thy1_2 = list(Ctrl=1.1, ACh=1.1, Atr=1.1),
  Yu_Thy1_3 = list(Ctrl=0.7, ACh=0.7, Atr=0.7),
  Yu_Thy1_4 = list(Ctrl=0.8, ACh=0.8, Atr=0.8),
  Yu_Thy1_5 = list(Ctrl=1.0, ACh=1.0, Atr=1.0),
...
animal.data.peaks = animal.data.binned %>%
  arrange(bin) %>%
  ddply(.(animal, exp), mutate,
        local.maximum = get.maxima(ztrace.shifted)$ispeak,
        peak.dur = get.maxima(ztrace.shifted)$peak.durs * bin.dur.sec,
        peak.start = get.maxima(ztrace.shifted)$peak.starts * bin.dur.sec, 
        peak.end = get.maxima(ztrace.shifted)$peak.ends * bin.dur.sec, 
        pop.event.thr = get.pop.event.thr(animal[1], exp[1])) %>%
  mutate(is.pop.event=local.maximum & (ztrace.shifted >= pop.event.thr))  %>%
  filter(is.pop.event)
```
...
pop.events.signal %>%
  rowwise() %>%
  dplyr::mutate(pop.event.thr = (get.pop.event.thr(animal, exp) - ztrace.min) * 0.5 + ztrace.min) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(animal, exp, bin) %>% 
  dplyr::summarise(ncells=n(), 
                   #nactive=sum(has.event),
                   #nactive2=sum(ztrace - ztrace.min > 0.5),
                   nactive=sum(ztrace > pop.event.thr),
                   event.dur=mean(peak.dur),
                   peak.zscore=mean(ztrace.mean) %>% round(2),
                   peak.trace.mean=mean(trace.mean) %>% round(2),
                   peak.trace.median=median(trace.mean) %>% round(2),
                   peak.strace.mean=mean(strace.mean) %>% round(2),
                   peak.strace.median=median(strace.mean) %>% round(2)) ->
  pop.active.df

DT::datatable(pop.active.df)
```
Z-score during population events
```{r}
pop.events.signal %>%
  rowwise() %>%
  dplyr::mutate(pop.event.thr = (get.pop.event.thr(animal, exp) - ztrace.min) * 0.5) %>%
  dplyr::ungroup() %>%
  ggplot() +
  geom_density(aes(x=ztrace - ztrace.min, color=exp, stat(count))) +
  facet_grid(. ~ animal, scales = 'free_x') + 
  coord_flip() +
  xlim(c(-2,7)) +
  geom_vline(aes(xintercept=pop.event.thr, color=exp)) +
  gtheme 
```
...
pop.events.signal %>%
  ...
  dplyr::summarise(ztrace.mean = median(ztrace),
                   ztrace.sd = sd(ztrace),
                   ztrace.min = min(ztrace.min),
                   pop.thr = min(pop.event.thr)) %>% 
  ggplot() +
  geom_density(aes(x=ztrace.mean - ztrace.min, color=exp, stat(count))) +
...
pop.active.summary = pop.active.df %>%
  ...
  dplyr::summarise(active.m=mean(nactive) %>% round(),
                   ...
                   mean.zscore = round(mean(peak.zscore), 1),
                   mean.trace = round(mean(peak.trace.mean), 2), 
                   median.trace = round(median(peak.trace.mean), 2), 
                   mean.strace = round(mean(peak.strace.mean), 2), 
                   median.strace = round(median(peak.strace.mean), 2), 
                   nevents=n(),
...
pop.active.summary %>%
  ...
  dplyr::summarise(events.dur.mean = mean(events.dur),
                   ...
                   peak.strace.mean = mean(median.strace),
                   peak.strace.sem = sem(median.strace))
...
X = events.participation.df %>%
  filter(animal %in% c('Necab_M8', 'Necab_M11', 'Necab_M13', 
                       'Yu_Thy1_2', 'Yu_Thy1_3', 'Yu_Thy1_4',
                       'Yu_Thy1_5', 'Yu_Thy1_7'))
  #filter(animal == 'Yu_Thy1_5') 
...
cor.sim.df$comparison = factor(cor.sim.df$comparison, 
                               levels=c('ACh - Ctrl', 'ACh - Atr', 'Ctrl - Atr'))

plot.2cor.similarity(cor.sim.df) +
    facet_wrap(. ~ animal, ncol = 3) 
ggsave(paste0(gen_imgs_dir, 'S4-cors_diff_faceted.pdf'), 
       device=cairo_pdf,
       width=width_2col, height=9, units='cm')
```

Are differences much different than 0?
```{r}
ks.results = list()
...
# Figure 5 - correlations across networks
```{r, warning=FALSE}
cors.change.df = data.frame()
cors.pval.df = data.frame()
for (animal_id in unique(filtered.all.data$animal)) {
  binned.data = filter(all.data.binned, animal == animal_id)
  res.kmeans.all = assign.clusters(binned.data)
  animal.data = add.cluster.to.all(binned.data)
  ...
  pval.ctrl = signif.shuffled.cors(shuffled.ctrl, cors.ctrl) %>% quantile(probs=0.95)
  pval.ach = signif.shuffled.cors(shuffled.ach, cors.ach) %>% quantile(probs=0.95)
  pval.atr = signif.shuffled.cors(shuffled.atr, cors.atr) %>% quantile(probs=0.95)
  animal.pval.df = data.frame(exp=c('Ctrl', 'ACh', 'Atr'),
                              pval=c(pval.ctrl[1], pval.ach[1], pval.atr[1]))
  animal.pval.df$animal = rep(animal_id, 3)
  
  animal.change.df = bind_rows(cors.ctrl, cors.ach, cors.atr)
  animal.change.df$animal = rep(animal_id, nrow(animal.change.df))
  if (nrow(cors.change.df) > 0) {
    cors.change.df = bind_rows(cors.change.df, animal.change.df)
  } else {
    cors.change.df = animal.change.df
  }
  
  cors.pval.df = bind_rows(cors.pval.df, animal.pval.df)
}
```

```{r}
cors.change.df$exp = factor(cors.change.df$exp, levels=c('Ctrl', 'ACh', 'Atr'))
cors.pval.df$exp = factor(cors.pval.df$exp, levels=c('Ctrl', 'ACh', 'Atr'))
cors.pval.df$animal = as.factor(cors.pval.df$animal)
cors.pval.df$exp = as.factor(cors.pval.df$exp)
summary.cors.change.df = cors.change.df %>%
  group_by(animal, exp) %>%
  dplyr::summarise(med.cor = median(vals),
                   sem.cor = sem(vals))
summary.cors.change.joined = summary.cors.change.df %>% left_join(cors.pval.df, by=c('exp', 'animal'))

summary.cors.change.joined %>%
  filter(pval <= 0.05) %>%
  ggplot(mapping=aes(x=exp, y=med.cor)) +
  geom_line(aes(group=animal), size=0.5) +
  gtheme +
  xlab('') +
  ylab('Median correlation') ->
g.cors.change.summary

g.cors.change.summary
```


# Figure5 - correlations in one network
```{r warning=FALSE}
animal_id = 'Necab_M13'
binned.data = filter(all.data.binned, animal == animal_id)
res.kmeans.all = assign.clusters(binned.data)
...
g.dist.all = plot_grid(
  ...
  plot.cor.dist(cor.atr, 'Atr', res.kmeans.all) + ggtitle('Atr') +
    ...
    labs(fill=''),
  ncol=3, rel_widths = c(1.25, 1.1, 1.6))
...
shuffled.ctrl = get.shuffled.cors(animal.data, 'Ctrl', nshuffles = 5)
shuffled.ach = get.shuffled.cors(animal.data, 'ACh', nshuffles = 5)
shuffled.atr = get.shuffled.cors(animal.data, 'Atr', nshuffles = 5)
shuffled.df = bind_rows(shuffled.ctrl, shuffled.ach, shuffled.atr)
shuffled.df$exp = factor(shuffled.df$exp, levels=levels(animal.data$exp))

cors.ctrl = get.cors.df(animal.data, 'Ctrl')
cors.ach = get.cors.df(animal.data, 'ACh')
cors.atr = get.cors.df(animal.data, 'Atr')
cors.df = bind_rows(cors.ctrl, cors.ach, cors.atr)
cors.df$exp = factor(cors.df$exp, levels=levels(animal.data$exp))

g.shuffled.cors = plot_grid(
  plot.shuffled.cors(shuffled.df, cors.df) + facet_grid(. ~ exp) + gtheme,
  g.cors.change.summary,
  ncol=2,
  labels=c('','C'), label_size=gtheme.label_size,
  rel_widths=c(2, 1.4) 
)
corr.change[animal_id] = get.corr.change(animal.data)
corr.changes.pct[[animal_id]] = calc.cor.changes.pct(animal.data)
...
animal.events.participation = filter(events.participation.df, animal == animal_id)
g.cluster.assignments.animal = plot.cluster.assignments(ungroup(animal.events.participation))
g.cors.diff.animal = plot.2cor.similarity(filter(cor.sim.df, animal == animal_id))

g.clus.reassignment.animal = plot_grid(g.cors.diff.animal + theme(legend.position = 'top'), 
                                       g.cluster.assignments.animal,
                                       ncol=2,
                                       rel_widths=c(1.5, 1.0),
                                       labels=c('', 'E'), label_size = gtheme.label_size)

g.cors.diff = plot.2cor.similarity(cor.sim.df)
g.clus.reassignment.all = plot_grid(g.cors.diff + theme(legend.position = 'top'), 
                                    g.cluster.assignments.all,
                                    ncol=2, 
                                    rel_widths = c(1.5, 1.0), 
                                    labels=c('','G'), label_size = gtheme.label_size)
 
g = plot_grid(grob(),#g.dist.all,
              g.shuffled.cors,
              g.clus.reassignment.animal,
              g.clus.reassignment.all,
              labels = c('A','B', 'D', 'F'),
              label_size = gtheme.label_size,
              ncol=1, 
              rel_heights = c(1.7, 1.7, 2.2, 2.2))

ggsave(paste0(gen_imgs_dir, '5-corrs-small.pdf'), 
       g,
       units='cm',
       device=cairo_pdf,
       dpi=200,
...
ggsave(paste0(gen_imgs_dir, '5-corrs-shuffled.pdf'), 
       g.shuffled.cors,
       units='cm',
       device=cairo_pdf,
       dpi=200,
...
ggsave(paste0(gen_imgs_dir, '4-autocorrelation.pdf'), 
       device=cairo_pdf,
       g.acorr,
       units='cm',
       width=width_1col,
...
ggsave(paste0(gen_imgs_dir, '3-population-activity_small.pdf'), 
       g.pop,
       device=cairo_pdf,
       units='cm',
       width=width_2col,
...
Activity raster for all networks
```{r}
...
ggsave(paste0(gen_imgs_dir, 'S3-1-pop_activity.png'), 
       all.activity.raster,
       dpi=300,
