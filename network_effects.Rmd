...
animal.data.peaks = animal.data.binned %>%
  ...
  ddply(.(animal, exp), mutate,
        local.maximum = c(0, diff(sign(diff(ztrace.mean)))==-2, 0)) %>%
  mutate(is.pop.event=local.maximum & (ztrace.mean > pop.event.thr * ztrace.mean.sd))  %>%
  filter(is.pop.event)
...
animal.data.binned %>%
  ...
  geom_hline(aes(yintercept= pop.event.thr * ztrace.mean.sd), linetype='dashed') +
  geom_point(data=subset(animal.data.peaks, animal==animal_name),
             aes(x=bin*bin.dur.sec, y=ztrace.mean + 0.3), shape=8, colour='red') +
  facet_grid(exp ~ .) +
...
left_join(all.data.binned, animal.data.peaks, by=c('date', 'animal', 'exp', 'bin')) %>% 
  filter(is.pop.event) %>% 
  group_by(animal, exp, bin) %>% 
  dplyr::summarise(ncells=n(), 
                   nactive=sum(has.event),
                   nactive2=sum(ztrace.max > (ztrace.mean + pop.event.thr * ztrace.mean.sd))) ->
  pop.active.df
...
pop.active.summary %>%
  group_by(exp) %>%
  dplyr::summarise('Median active neurons (%)' = round(median(active.pct.m)),
                   'std (%)'=round(sd(active.pct.m)),
                   'Median freq of population events (Hz)'=round(median(events.freq),3),
                   'std (Hz)'=round(sd(events.freq),2))

ggplot(pop.active.summary, aes(x=exp,y=active.pct.m)) +
  geom_line(aes(group=animal), alpha=0.4) +
  geom_point(aes(color=animal, size=nevents), alpha=0.6) +
  gtheme +
  xlab('') + ylab('Participation in population events (%)') +
  ylim(c(0,100))
