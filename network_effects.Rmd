...
for (animal_id in unique(filtered.all.data$animal)) {
  ...
  g.shuffled.cors = plot_grid(
    ...
    plot.shuffled.cors(shuffled.ach, cors.ach, includeYaxis=F) + gtheme,
    plot.shuffled.cors(shuffled.atr, cors.atr, includeYaxis=F) + gtheme,
    ncol=3)
...
g.shuffled.cors = plot_grid(
  ...
  plot.shuffled.cors(shuffled.atr, cors.atr) + gtheme_noyaxis,
  g.cors.change.summary,
  ncol=4,
  rel_widths=c(1.5, 1.0, 1.0, 1.7))
corr.change[animal_id] = get.corr.change(animal.data)
corr.changes.pct[[animal_id]] = calc.cor.changes.pct(animal.data)
...
ggsave(paste0(gen_imgs_dir, '4-corrs.png'), 
       g,
       units='cm',
       #device=cairo_pdf,
       dpi=200,
...
g.acorr.change = peak.autocorr.lags %>%
  ...
  #geom_point(aes(size=1-peak.signif), shape=21, stroke=px2pt) + 
  geom_dotplot(aes(y=peak.time, fill=peak.signif >= 0.05), dotsize=2,
              binaxis = "y", stackdir = "center", position = "identity", binwidth=1) +
  #scale_fill_discrete(c('black', 'grey60')) +
...
# signif = c(1, pnorm(r[-1] / se[-length(se)])
...
ggsave(paste0(gen_imgs_dir, '3-autocorrelation.png'), 
       g.acorr,
       units='cm',
       width=width_2col,
       height=5)
