...
#zscore = function(x) {
#  x = (x - mean(x)) / sd(x)
#  lowval = quantile(x, probs=0.05)[1]
#  return(x - lowval)
#}
...
```{r}
plot.cluster.assignments = function(clust.df, clust.distance=0) {
  
 if (clust.distance==0) {
   clust.distance = nrow(clust.df)/2
 }
 ...
 ordinal.df = group_by(my.clust.df, Ctrl) %>%
  ...
  ordinal.df$exp = factor(ordinal.df$exp, levels = levels(clust.df$exp))
  s = left_join(clust.df, ordinal.df, by=c('cell_id', 'exp'))
  ...
  s %>%
    ...
    geom_line(aes(x=exp,y=row_ordinal, group=cell_id), alpha=0.1, size=1) +
    gtheme +
...
shapiro.test(subset(pop.active.summary, exp=='Ctrl')$mean.zscore)
shapiro.test(subset(pop.active.summary, exp=='ACh')$mean.zscore)
t.test(mean.zscore ~ exp, subset(pop.active.summary,exp != 'Atr'), paired=TRUE)

g.pop.event.peak = filter(pop.active.summary, exp != 'Atr') %>%
  ggplot() +
  geom_line(aes(x=exp, y=mean.zscore, group=animal)) +
  gtheme +
  xlab('') +
  ylab('Mean Peak (z-score)')
...
g.cluster.assignments.all = plot.cluster.assignments(ungroup(X), clust.distance=220) 
...
# Figure 4 - correlations across networks
```{r, warning=FALSE}
...
# Figure 4 - correlations in one network
```{r warning=FALSE}
animal_id = 'Necab_M13'
binned.data = filter(all.data.binned, animal == animal_id)
res.kmeans.all = assign.clusters(binned.data)
...
gtheme_noyaxis = gtheme +
    theme(axis.line.y = element_blank(),
          axis.text.y = element_blank(),
          axis.title.y = element_blank(),
          axis.ticks.y = element_blank())
#g.shuffled.cors = plot_grid(
#  plot.shuffled.cors(shuffled.ctrl, cors.ctrl) + gtheme + xlab(''),
#  plot.shuffled.cors(shuffled.ach, cors.ach) + gtheme_noyaxis,
#  plot.shuffled.cors(shuffled.atr, cors.atr) + gtheme_noyaxis + xlab(''),
#  g.cors.change.summary,
#  ncol=4,
#  rel_widths=c(1.8, 1.0, 1.0, 2.2))
...
g = plot_grid(g.dist.all,
              ...
              label_size = gtheme.label_size,
              ncol=1, 
              rel_heights = c(1.7, 1.7, 2.2))

ggsave(paste0(gen_imgs_dir, '4-corrs.pdf'), 
       g,
       units='cm',
       device=cairo_pdf,
       dpi=200,
...
ggsave(paste0(gen_imgs_dir, '3-autocorrelation.pdf'), 
       device=cairo_pdf,
       g.acorr,
       units='cm',
       width=width_1col,
